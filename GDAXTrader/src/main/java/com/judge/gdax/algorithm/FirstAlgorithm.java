...
public class FirstAlgorithm implements Algorithm
{
	...
	private static final int LONG_SLOPE = 5;
	...
	private boolean buy()
	{
		// if SHORT_SLOPE > LONG_SLOPE AND positive or close to positive AND no pending
		// sells, then buy
		if (wallet.amWaitingToBuy() && book.getSlope(SHORT_SLOPE) > book.getSlope(LONG_SLOPE)
				&& book.getSlope(SHORT_SLOPE) > -0.000001)
		{
			try
			{
				orderService.marketBuy(USD_LIMIT / currentPriceUsd);
	...
	{
		// if SHORT_SLOPE < LONG_SLOPE AND greater than (the buy
		// price - 1%), then sell
		if (!wallet.amWaitingToBuy() && book.getSlope(SHORT_SLOPE) < book.getSlope(LONG_SLOPE)
				&& book.getSlope(SHORT_SLOPE) < 0.00000
				&& currentPriceUsd > wallet.getLastBuyUsdPrice() + wallet.getLastBuyUsdPrice() * 0.006)
		{
			wallet.setWaitingToBuy(true);

			try
			{
				orderService.marketSell(wallet.getLastBuyCrypto());
